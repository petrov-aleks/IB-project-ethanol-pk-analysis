...
occasion_1 <- Ammon_clean %>% 
  ...
  group_by(subject_id) %>% 
  ggplot()+
  geom_line(aes(x = time_min, y = conc_g_per_L, group = subject_id, colour = subject_id)) +
  labs(x = "Время, мин",
       y = "Концентрация г/л",
       title = "Occasion 1")+
...
summ_1 <- ggplot(summ_ammon_1) +
  geom_errorbar(aes(x = factor(time_min),
                    ymin = mean_conc - sd_conc,
                    ymax = mean_conc + sd_conc),
                width = .2)+
  geom_point(aes(x = factor(time_min), y = mean_conc)) +
  labs(x = "Время, мин",
       y = "Концентрация, г/л")+
  scale_y_continuous(
