...
```{r upload, echo=FALSE}
read_csv("data/raw/Vestal-1977-elderly.csv") %>% 
  rename_with(tolower) -> data_vestal_e

# read_csv("data/raw/Vestal-1977-young.csv") %>%
#   rename_with(tolower) -> data_vestal_y
# Можно сразу все вместе взять


data_vestal_e <- data_vestal_e %>% 
  mutate(
    ...
    id_unified = paste("VE", route, subject_id, sep = "-")
  ) %>% 
  select(-cmax_mg_dl) %>% 
  relocate(id_unified, .before = subject_id) %>% 
  relocate(cmax, .before = dose_g_per_kg)

str(data_vestal_e)
```

```{r plot-conc-time-ind, fig.width=16, fig.height=9, echo=FALSE, warning=FALSE, message=FALSE}
ggpairs(data_vestal_e[, "cmax", "age", "wt_kg", "bsa_m2", ])
```
