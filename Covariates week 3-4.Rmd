...
ggplot(Kozawa_2007, aes(x = time_min, y = conc_g_per_L)) +
  geom_line(aes(group = dose_g_per_kg,
                color = dose_g_per_kg %>% as.factor())) +
...
# Аллометрическое скалирование на данных крыс
...
rat_data_pk <- rat_data_pk %>% 
  mutate(V_max_0_order = Beta * 60 * V_d_0_order) %>% # г / кг / час
  relocate(V_max_0_order, .after = V_d_0_order)

V_allom_scale <- function(V_rat, # в л, абсолютный объём
                          rat_weight, # в кг
                          human_weight = 1,
                          n = 1) # в кг, по умолчанию считаем на 1 кг
{
...
rat_data_pk %>%  
  mutate(Vd_abs_rat = (V_d_0_order * BW_g / 1000), # L
    Vd_abs_hum = V_allom_scale(Vd_abs_rat, BW_g / 1000, 72),
    V_d_0_order = V_d_0_order) %>% 
  select(study_id, dose_g_per_kg, route, BW_g, V_d_0_order, Vd_abs_rat, Vd_abs_hum) %>% 
  mutate(across(where(is.numeric), ~round(.x, 2))) %>% 
  rename("Исследование" = study_id,
         "Доза, г / л" = dose_g_per_kg,
         "Путь введения" = route,
         "Масса крысы, г" = BW_g,
         "Vd крысы, л / кг" = V_d_0_order,
         "Vd крысы абс., л" = Vd_abs_rat,
         "Vd человека абс., л" = Vd_abs_hum) %>% 
  flextable() %>% 
  flex_default_fun() %>% 
  add_header_row(values = "Аллометрическое шкалирование Vd", colwidths = 7)

```

```{r, message=FALSE}
order_0_df %>% 
  filter(!is.na(wt_kg) & !is.na(V_d_0_order)) %>% 
  mutate(Vd_abs = V_d_0_order * wt_kg) %>% 
  select(study_id, subject_id, dose_g_per_kg, route, wt_kg, V_d_0_order, Vd_abs) %>% 
  group_by(study_id, route) %>% 
  summarise(across(where(is.numeric), ~mean(.x))) %>% 
  mutate(across(where(is.numeric), ~round(.x, 2))) %>% 
  rename("Исследование" = study_id,
         "Доза, г / л" = dose_g_per_kg,
         "Пусть введения" = route,
         "Вес, кг" = wt_kg,
         "Vd человека, л / кг" = V_d_0_order,
         "Vd человека абс., л" = Vd_abs) %>% 
  flextable() %>% 
  flex_default_fun() %>% 
  add_header_row(values = "Усреднённые данные по Vd человека", colwidths = 6)
```
...
rat_data_pk %>%  
  mutate(Vmax_abs_rat = (V_max_0_order * BW_g / 1000), # g/h
    Vmax_abs_hum = V_allom_scale(Vmax_abs_rat, BW_g / 1000, 72, n = 0.81)) %>% 
  filter(!is.na(V_max_0_order)) %>% 
  select(study_id, dose_g_per_kg, route, BW_g, V_max_0_order, Vmax_abs_rat, Vmax_abs_hum) %>% 
  mutate(across(where(is.numeric), ~round(.x, 2))) %>% 
  rename("Исследование" = study_id,
         "Доза, г / л" = dose_g_per_kg,
         "Путь введения" = route,
         "Масса крысы, г" = BW_g,
         "Vmax крысы, г / кг / час" = V_max_0_order,
         "Vmax крысы абс., г / час" = Vmax_abs_rat,
         "Vmax человека абс., г / час" = Vmax_abs_hum) %>% 
  flextable() %>% 
  flex_default_fun() %>% 
  add_header_row(values = "Аллометрическое шкалирование Vmax", colwidths = 7) %>% 
  merge_v(j = c(1, 3, 4))
...
order_0_df %>% 
  filter(!is.na(wt_kg) & !is.na(V_d_0_order)) %>% 
  mutate(Vmax = Beta * 60 * V_d_0_order) %>% 
  mutate(Vmax_abs = Vmax * wt_kg) %>% 
  select(study_id, subject_id, dose_g_per_kg, route, wt_kg, Vmax, Vmax_abs) %>% 
  group_by(study_id, route) %>% 
  summarise(across(where(is.numeric), ~mean(.x))) %>% 
  mutate(across(where(is.numeric), ~round(.x, 2))) %>% 
  rename("Исследование" = study_id,
         ...
         "Путь введения" = route,
         "Вес, кг" = wt_kg,
         "Vmax человека, г / кг / час" = Vmax,
         "Vmax человека абс., г / час" = Vmax_abs) %>% 
  flextable() %>% 
  flex_default_fun() %>% 
  add_header_row(values = "Усреднённые данные по Vmax человека", colwidths = 6)
```
...
rat_data_pk %>%  
  mutate(Beta = Beta * 60, # g/L/h
         Beta_hum = V_allom_scale(Beta, BW_g / 1000, 72, n = -0.25)) %>% 
  filter(!is.na(Beta)) %>% 
  select(study_id, dose_g_per_kg, route, BW_g, Beta, Beta_hum) %>% 
  mutate(across(where(is.numeric), ~round(.x, 2))) %>% 
  rename("Исследование" = study_id,
         "Доза, г / л" = dose_g_per_kg,
         "Путь введения" = route,
         "Масса крысы, г" = BW_g,
         "Beta крысы, г / л / час" = Beta,
         "Beta человека абс., г / л / час" = Beta_hum) %>% 
  flextable() %>% 
  flex_default_fun() %>% 
  add_header_row(values = "Аллометрическое шкалирование Beta", colwidths = 6)
...
order_0_df %>% 
  filter(!is.na(wt_kg) & !is.na(V_d_0_order)) %>% 
  mutate(Beta = Beta * 60 * V_d_0_order) %>% 
  select(study_id, subject_id, dose_g_per_kg, route, wt_kg, Beta) %>% 
  group_by(study_id, route) %>% 
  summarise(across(where(is.numeric), ~mean(.x))) %>% 
  mutate(across(where(is.numeric), ~round(.x, 2))) %>% 
  rename("Исследование" = study_id,
         ...
         "Путь введения" = route,
         "Вес, кг" = wt_kg,
         "Beta человека, г / л / час" = Beta) %>% 
  flextable() %>% 
  flex_default_fun() %>% 
  add_header_row(values = "Усреднённые данные по Beta человека", colwidths = 5)
```
