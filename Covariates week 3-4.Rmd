...
Wilkinson_PO_clean %>%
  select(subject_id, conc_g_per_L, time_min, occasion) %>% 
  mutate(occasion = occasion %>% as.factor) %>% 
  ggplot(aes(x = time_min, y = log(conc_g_per_L))) +
  geom_line(aes(color = occasion, group = occasion), linewidth = 2) +
  geom_point(aes(group = occasion)) +
  geom_hline(yintercept = log(0.2), linetype = "dashed", color = "darkred") +
  facet_wrap(subject_id~.)+
  scale_x_continuous(breaks = seq(0, 420, by = 50))+
  scale_y_continuous(breaks = seq(0, 1, by = 0.25))+
...
pharma_fun <- function(conc, t, dose, k = 1, l = 0, order_1_calc = FALSE) {
  # Моделирование для подсчёта beta, C0, Vd 0 порядка
  t_sample_0 <- {{t}}[(first(which.max({{conc}})) + k):(length({{t}})-l)]
  conc_sample_0 <- {{conc}}[(first(which.max({{conc}})) + k):(length({{conc}})-l)]
  dose <- first({{dose}})
  model <- lm(conc_sample_0 ~ t_sample_0)
  ...
  if (order_1_calc == TRUE) {
    t_sample_1 <- {{t}}[(length({{t}})-l):length({{t}})]
    conc_sample_1 <- {{conc}}[(length({{t}})-l):length({{t}})]
    indexes <- conc_sample_1 > 0
    model_1 <- lm(log(conc_sample_1[indexes]) ~ t_sample_1[indexes])
...
Jones_1984_clean %>% 
  filter(conc_g_per_L < 0.25 & time_min > 50)
...
Ammon_1996_clean %>%
  filter(conc_g_per_L < 0.21 & time_min > 50)
rm(Ammon_1996_clean)
```
...
```{r, fig.height=7, fig.width=11, dpi=300}
t_max_coef %>% 
ggplot(aes(y = `Variable`)) +
  geom_linerange(aes(xmin = `95%CI low`, xmax = `95%CI high`), linewidth = 1.2, color = "darkorange2") +
    geom_point(aes(x = `β estimate`), shape = 21, size = 3, color = "darkorange2", fill = "white") +
  theme_classic() +
...
``` {r}
ggpredict(t_max_full_m, terms = c("food", "route")) %>% plot(show_data = TRUE, log_y = FALSE, jitter = .01) + labs(subtitle = "Adjusted R-squared:  0.497")
```

``` {r}
ggpredict(t_max_full_m, terms = c("dose_g_per_kg", "route")) %>% plot(show_data = TRUE, log_y = FALSE, jitter = .01) + labs(subtitle = "Adjusted R-squared:  0.497")
```


```{r, fig.width=7, fig.height=5, dpi=300}
plot_coefs(t_max_full_m, omit.coefs = NULL, scale = TRUE, ci_level = 0.95, plot.distributions = FALSE, inner_ci_level = .1, colors = "CUD") + labs(title = "Beta coeffecients with 95% CI")
```
