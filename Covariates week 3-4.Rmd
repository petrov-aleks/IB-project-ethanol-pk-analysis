...
Ammon_1996_pk <- Ammon_1996_clean %>% 
  ...
  mutate(across(c(subject_id, occasion), ~as.integer(.x)))
...
Ammon_1996_pk <- Ammon_1996_clean %>%
  ...
  mutate(subject_id = str_c("A-IV-", case)) %>% 
  select(-case)
rm(Ammon_1996_clean)
```
...
Yelland_2008_pk <- Yelland_2008_clean %>%
  ...
  mutate(subject_id = str_c("Y-", case)) %>% 
  select(-case)
...
t.test(log(Cmax) ~ route, data = main_df)

t.test(log(Cmax) ~ food, data = main_df)
