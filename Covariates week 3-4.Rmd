...
# Аллометрическое шкалирование с использованием фармакокинетических параметров крыс

## Observed - Predicted (Wilkinson IV)

```{r rat} 
# Перевод в другую размерность
...
```{r allometric_func}
allom_scale <- function(V_rat, # в л, абсолютный объём
                        rat_weight, # в кг
                        human_weight = 1, # в кг, по умолчанию считаем на 1 кг
                        n = 1)
{
...
```{r allometric}
# Подготовка датасета для аллометрии
...
# Только по датасету Вилкиксон (человеческие дозы 0,5 - 0,62)
...
## IV allometric scaling (main df)

```{r allometric_IV}
# Датасет на все внутривенные значения
...
```{r main_dose_IV}
# Наблюдаемые значения и средняя доза для выборки
...
## PO allometric scaling (750 mg per kg)
...
## PO allometric scaling (3000 mg per kg)
