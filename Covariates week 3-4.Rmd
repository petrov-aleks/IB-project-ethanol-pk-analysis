...
Нужно добавить food, чтобы показать его влияние на на AUC/Cmax/beta

```{r}
# заменим site из предыдущей модели fit_AUC_mod_int на food, т.к. вместе их не добавить

fit_AUC_mod_with_food <- lm(log(AUC) ~ dose_g_per_kg*route + food, order_0_df)

summ(fit_AUC_mod_with_food, confint = TRUE, digits = 3)

# anova(fit_AUC_mod_int, fit_AUC_mod_with_food)
```
...
# строим график с ковариатами  и их ДИ
plot_model(fit_AUC_mod_with_food, show.values = TRUE,  show.intercept = TRUE, value.offset = 0.3, axis.labels = c("Dose (g per kg) x Route (PO)", "Food status (fed)", "Route (PO)", "Dose (g per kg)","Intercept")) +
...
```{r}
# достаем значения CI для подписей графика
...
# # строим табличку
# coeftest(fit_AUC_mod_with_food, vcov. = vcovHC, type = "HC3") %>%
...
plot_model(Cmax_lm2_v2, show.values = TRUE,  show.intercept = TRUE, value.offset = 0.3, digits = 3) +
  geom_hline(yintercept = 0, color = "gray3", size = 0.5) +
...
```{r}
fit_Beta_1 <- lm(log(Beta) ~ log(dose_g_per_kg) * route + food, data = order_0_df_factor)
...
plot_model(fit_Beta_1, show.values = TRUE,  show.intercept = TRUE, value.offset = 0.3,  axis.labels = c("Dose (g per kg) x Route (PO)", "food (fed)","Route (PO)", "Dose (g per kg)", "Intercept")) +
labs(title = "Regression coefficients for Beta with 95% CI", 
       subtitle = "log(Cmax) ~ dose_g_per_kg + route + food + sex + age") +
  theme_nice()+
  theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5))
```
