...
# 5. Основной регрессионный анализ# 5. Основной регрессионный анализ

строим модели по общему датасету order_0_df, одну модель по параметрам, которые есть у всех "датасетов", и еще можно другие модели, добавляя возраст и/или пол и т д

+ еще одну модель , в которую добавить еще и площадью тела и сухую массу тела

Примерные спецификации моделей
1) log(AUC) ~ dose + route + food + site + study (?)
2) ~ ... + пол + возраст
3) ~ площадь поверхности тела + сухая масса тела
У каждого свой показатель, можно попробовать включить взаимодействия переменных
...
```
Визуально выглядит лучше, но предикторов очень много, посчитал bmi и bsa, попробовал по всякому включить в модель
