...
```{r basic lms}
fit_Vd_1 <- lm(log(V_d_0_order) ~ route + food + site, data = order_0_df_factor)

summary(fit_Vd_1)

cat("\n===============================================================\n")

fit_Vd_3 <- lm(log(V_d_0_order) ~ route, data = order_0_df_factor)

summary(fit_Vd_3)

cat("\n===============================================================\n")

fit_Vd_4 <- lm(log(V_d_0_order) ~ route + site, data = order_0_df_factor)

summary(fit_Vd_4)

cat("\n===============================================================\n")

fit_Vd_5 <- lm(log(V_d_0_order) ~ route + food, data = order_0_df_factor)
...
```{r}
fit_Vd_3_sub <- lm(log(V_d_0_order) ~ 1,  data = order_0_df_factor)

anova(fit_Vd_3, fit_Vd_3_sub)
```

```{r model check, fig.width = 16, fig.height = 8}
check_model(fit_Vd_3)
```

```{r}
fit_Vd_3_add <- lm(log(V_d_0_order) ~ route + wt_kg * sex, data = order_0_df_factor)
fit_Vd_3_add2 <- lm(log(V_d_0_order) ~ route + wt_kg + sex, data = order_0_df_factor)

anova(fit_Vd_3_add, fit_Vd_3_add2)

summary(fit_Vd_3_add2)

fit_Vd <- lm(log(V_d_0_order) ~ wt_kg + sex, data = order_0_df_factor)
...
```
```{r}
ggpredict(fit_Vd, terms = c("wt_kg", "sex"), ci_level = 0) %>% plot(show_data = TRUE, log_y = FALSE, jitter = .01) + labs(subtitle = "Adjusted R-squared:  0.2632") + xlab("Weight (kg)")
```
```{r}
plot_model(fit_Vd, show.values = TRUE,  show.intercept = TRUE, value.offset = 0.3,  axis.labels = c("Weight (kg)", "Sex", "Intercept"), title = "Regression coeffecients with 95% CI") +
  theme_nice()
```
...
```{r}
fit_Beta_1 <- lm(log(Beta) ~ log(dose_g_per_kg) * route, data = order_0_df_factor)

summary(fit_Beta_1)
```

```{r}
fit_Beta_1_sub <- lm(log(Beta) ~ 1,  data = order_0_df_factor)
...
plot_model(fit_Beta_1, show.values = TRUE,  show.intercept = TRUE, value.offset = 0.3,  axis.labels = c("Dose (g per kg)", "Route (PO)", "Dose (g per kg) x Route (PO)", "Intercept"), title = "Regression coeffecients with 95% CI") +
  theme_nice()
```
## 6.4. Построение регрессионных моделей по C_0
...
Линии плотности моделей и наблюдений почти совпадают. Это означает, что модель в целом адекватно воспроизводит распределение C_0_0

2) Linearity

зелёная сглаженная линия близка к горизонтали, но виден лёгкий систематический тренд на правом конце (при больших fitted). 
Сильного нарушения линейности нет. возможна не хватает эффекта взаимодействия или нелинейного эффекта одного из предикторов.
...
видны несколько наблюдений, которые помечены номерами (78, 92, 62, 95, 67), заметнее всех

5) Normality of residuals

Q–Q график выглядит приемлемо

6) Collinearty VIF
dose_g_per_kg, route, study_id - High ≥ 10

Попробую убрать лишние показатели, например в каждом исследовании 1 уровень food
...
Убрали коллинеарность

```{r С_0_0_order-lm2-v1-v2-part1}
order_0_df
C_0_0_lm2_v1 <- lm(log(C_0_0_order) ~ study_id + sex+age , data = order_0_df)
summary(C_0_0_lm2_v1) #sex и age зависит от study_id, нет смысла смотреть по всем данным


df_sa <- order_0_df %>% filter(!is.na(sex), !is.na(age), !is.na(C_0_0_order)) #по факту вилкинсон и Thierauf, 18 наблюдений

df_sa <- df_sa %>%  mutate(bmi = wt_kg / (bh_cm/100)^2, 
         bsa_m2 = sqrt(bh_cm * wt_kg / 3600)) #bsa в модели заходит лучше
...
изуально выглядит лучше, но предикторов очень много, посчитал bmi и bsa, попробовал по всякому включить в модель
...
## 6.5. Построение регрессионных моделей по Vd
...
## 6.6. Построение регрессионных моделей по Beta
...
# 7. Интерпретация моделей. Влияние факторов на ФК-параметры

## 7.1 Влияние массы тела и площади поверхности тела на ФК-параметры
...
fit_Vd %>% broom::tidy(conf.int = TRUE) %>% filter(term == "wt_kg") %>% 
  ...
  flextable() %>%
  add_header_row(., values = "Оценка коэффициента массы тела в модели Vd", colwidth = 6) %>% 
  flex_default_fun()
```
...
lm(V_d_0_order ~ bsa_m2 + sex, data = order_0_df) %>% 
  ...
  mutate(across(c(where(is.numeric), -p.value), ~round(.x, 3))) %>% 
  mutate(p.value = gtsummary::style_pvalue(p.value),
         term = "Площадь поверхности тела, м²") %>% 
  select(-statistic) %>% 
  slice_tail(n = 1) %>% 
  rename(
    `Показатель` = term,
  ...
  flextable() %>%
  add_header_row(., values = "Оценка коэффициента площади поверхности тела в модели Vd", colwidth = 6) %>% 
  flex_default_fun()
```
...
Cmax_lm2_v3 %>% 
  ...
  flextable() %>%
  add_header_row(., values = "Оценка коэффициента сухой массы тела в модели Cmax", colwidth = 6) %>% 
  flex_default_fun()
```

При увеличении массы тела на 1 кг Vd уменьшается на 0.004 (95%ДИ[0.001; 007]) л/кг, при увеличении площади поверхности тела на 1 $м^2$ Vd уменьшается на 0.311 (95%ДИ[0.032; 0.589]) л/кг.

При увеличении сухой массы тела на 1 кг Cmax уменьшается на 0.0086 (95%ДИ[0.0002; 0.017]) г/л.

## 7.2 Влияние пола на ФК-параметры

Пол не влияет значимо на AUC (Мужской пол 95%ДИ[-0.3108; 0.1754]), на Cmax (Мужской пол 95%ДИ[-0.1505; 0.326])

У субъектов мужского пола объем распределения на 0,106 л/кг выше, чем у женщин при прочих равных (95%ДИ[-0.0155; 0.1964])).
...
fit_Vd %>% 
  ...
  flextable() %>%
  add_header_row(., values = "Оценка коэффициента пола в модели Vd", colwidth = 6) %>% 
  flex_default_fun()
