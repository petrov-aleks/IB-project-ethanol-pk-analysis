...
```{r fit_lm}
fit1 <- lm(log(Cmax) ~ dose_g_per_kg, main_df)
fit2 <- lm(log(Cmax) ~ age, main_df)
fit3 <- lm(log(Cmax) ~ wt_kg, main_df)
fit4 <- lm(log(Cmax) ~ sex, main_df)
fit5 <- lm(log(Cmax) ~ study_id, main_df)
...
# 
# 
# grid.arrange(pfit1, pfit2, pfit3, pfit4, pfit5, ncol = 2, top = textGrob("Diagnosis of linearity by fitted value", gp = gpar(fontsize = 24, fontface = "bold")))
```


```{r}
autoplot(fit1) +theme_minimal() # dose
autoplot(fit2) +theme_minimal() # age
autoplot(fit3) +theme_minimal() # wt_kg
autoplot(fit4) +theme_minimal() # sex
autoplot(fit5) +theme_minimal() # study_id
